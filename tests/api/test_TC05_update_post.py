...
from data.generators.post_data_generators import PostData as pd
...
def test_update_post(data_and_create_post, posts_api):
    ...
    new_data = pd.generate_post_title_content()

    with allure.step("1. Обновление поста через API"):
        posts_api.update_post(post_id, new_data)

    with allure.step("2. Поиск измененного поста в БД"):
        db_response = pc.get_post_data(post_id)
